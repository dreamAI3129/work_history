...
def addSkill(driver, inp, skill, field="skills-input", end=2):
    ...
    # for i in skill:
    driver.find_element(
        By.CSS_SELECTOR, f'input[aria-labelledby="{field}"]').send_keys(skill)
    sleep(0.1)
...
def configLast(driver, country, street, city, birthday, phone, photo):
    ...
    waitInfinite(lambda: driver.find_element(By.CSS_SELECTOR, 'input[type="file"]').send_keys(
        os.path.join(os.getcwd(), 'avatars', photo)))
    waitInfinite(lambda: driver.execute_script(
...
with open('./profiles/' + name + '.json', 'r', encoding="utf-8-sig") as f:
    profile = json.load(f)

excel_file_name=input('Input email file name :')
emailFile = './'+ excel_file_name +'.xlsx'
count_s=input('Input count of what you want to complete the profile :')

excel = openpyxl.load_workbook(emailFile)
sheet = excel.active
# rowCount = sheet.max_row
rowCount=int(count_s)
...
while row < rowCount+1 :
    ...
    if sheet.cell(row=row, column=2).value=="verified" :
        if sheet.cell(row=row, column=3).value!="completed" :
            email = sheet.cell(row=row, column=1).value
            sheet.cell(row=row, column=3).value="completed"
            excel.save(emailFile)

            if sheet.cell(row=row, column=4).value == "" :
                load_profile=input("Can not find profile  name on this row !!  ---> Input it here")
            else : 
                load_profile=sheet.cell(row=row, column=4).value

            with open('./profiles/' + load_profile + '.json', 'r', encoding="utf-8-sig") as f:
                profile = json.load(f)

        elif sheet.cell(row=row, column=3).value == "completed":
            rowCount=rowCount+1
            row=row+1
            continue
    else :
        if danger_hopping == 0 :
            ...
            print("Run out of verified emails!!!!")
            break
        else :
            danger_hopping=danger_hopping-1
            row=row+1
            rowCount=rowCount+1
            print("Found an email that is not verified or not available!! One step over")
            continue
    ...
    sleep(2)
    try:
        driver.find_element(By.CSS_SELECTOR, "div[id=\"username-message\"]")
        ...
        waitInfinite(lambda: driver.find_element(By.CSS_SELECTOR,
                    "button[data-test=\"next-button\"]").click())
        sleep(2)
        for i in profile['work_experience']:
        ...
        waitInfinite(lambda: driver.find_element(By.CSS_SELECTOR,
                    'input[data-test="currency-input"]').send_keys(str(profile['hour_rate'])))
        ...
        # configLast(
        #     driver,
        #     profile['photo_others']['location'],
        #     profile['photo_others']['street'],
        #     profile['photo_others']['city'],
        #     random_birthday, # profile['birthday'],
        #     str(random.randint(0, 999999999)),  # profile['phone'],
        #     random.choice(profile['photo_others']['avatar'])
        # )
        action = webdriver.ActionChains(driver)
        ...
        sht.cell(row=row, column=1).value = email
